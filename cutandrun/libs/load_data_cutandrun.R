...
bcbio_qc_cutandrun_testdata <- function(){
  # if using example data to render report, download peaks from github
  api_url <- "https://api.github.com/repos/bcbio/bcbioR-test-data/contents/cutandrun/03_peak_calling/04_called_peaks/macs2/"
...
load_metrics <- function(multiqc_data_dir){
  ...
  bowtie2 <- read_tsv(file.path(multiqc_data_dir, 'multiqc_bowtie2.txt')) %>% clean_names() %>%
    dplyr::select(sample, mapped_reads = paired_aligned_one, mapped_reads_pct = overall_alignment_rate)
  
  # phantom <- read_tsv(file.path(multiqc_data_dir, 'multiqc_phantompeakqualtools.txt')) %>% clean_names() %>%
  #   dplyr::select(sample, nsc, rsc)
  frip <- read_tsv(file.path(multiqc_data_dir, 'multiqc_primary_fripscore_plot.txt')) %>% clean_names() %>%
    dplyr::rename(frip = peak_f_ri_p_score) %>% filter(!is.na(frip))
  peak_count <- read_tsv(file.path(multiqc_data_dir, 'multiqc_primary_peakcounts_plot.txt')) %>% clean_names() %>% 
    filter(!is.na(peak_count))
  nrf <- read_tsv(file.path(multiqc_data_dir, 'mqc_picard_deduplication_1.txt')) %>% clean_names() 
  
  if ('dupicate_pairs_optical' %in% names(nrf)) {
    nrf <- nrf %>%
      mutate(nrf = unique_pairs / (duplicate_pairs_optical + duplicate_pairs_nonoptical + unique_pairs)) %>%
      dplyr::select(sample, nrf)
  } else {
    nrf <- nrf %>%
      mutate(nrf = unique_pairs / (duplicate_pairs_nonoptical + unique_pairs)) %>%
      dplyr::select(sample, nrf)
  }
  
  metrics <- full_join(fastqc, bowtie2) %>% full_join(frip) %>% 
...
load_coldata <- function(coldata_fn, column=NULL, numerator=NULL, denominator=NULL, subset_column = NULL, subset_value = NULL){
  coldata <- read.csv(coldata_fn) %>%
    mutate(sample = paste0(group, '_R', replicate)) %>%
...
load_peaks <- function(peaks_dir){
  ...
  peaks_dir_files <- list.files(peaks_dir)
  if(any(grepl('broadPeak', peaks_dir_files))){
  ...
  peaks_all <- lapply(peaks_fns, function(fn) {
    ...
    if(nrow(peaks) > 0){
      peaks_df <- data.frame(seqnames = as.character(peaks$X1), start = peaks$X2, end = peaks$X3,
                             peak_enrichment = peaks$X7, peak_rank = rank(dplyr::desc(peaks$X7))) %>% 
        dplyr::arrange(peak_rank)
      return(peaks_df)
    }
  }) %>% bind_rows(.id = 'sample')
  peaks_all$sample <- gsub('\\.macs2', '', peaks_all$sample)
  peaks_all$sample_group <- gsub('_R[0-9]+', '', peaks_all$sample)
